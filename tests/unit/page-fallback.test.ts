...
describe('fetchOpenLibraryPages', () => {
  it('returns the first numeric number_of_pages from editions', async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({
        entries: [{}, { number_of_pages: 288 }, { number_of_pages: 999 }],
      }),
    );
    expect(
      await fetchOpenLibraryPages(
        '/works/OL1W',
  ...
  it('parses textual pagination when number_of_pages absent', async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({ entries: [{ pagination: '384 p.' }] }),
    );
    expect(
      await fetchOpenLibraryPages(
        '/works/OL1W',
  ...
  it('returns null for non-work keys', async () => {
    ...
    expect(
      await fetchOpenLibraryPages('OL1M', fetchImpl as unknown as typeof fetch),
    ).toBeNull();
...
describe('fetchGoogleBooksPages', () => {
  it('returns volumeInfo.pageCount', async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({ volumeInfo: { pageCount: 512 } }),
    );
    expect(
      await fetchGoogleBooksPages(
        'abc123',
        fetchImpl as unknown as typeof fetch,
      ),
    ).toBe(512);
  ...
  it('returns null when pageCount missing or zero', async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({ volumeInfo: { pageCount: 0 } }),
    );
    expect(
      await fetchGoogleBooksPages(
        'abc123',
  ...
  it('returns null for empty id without calling fetch', async () => {
    const fetchImpl = vi.fn();
    expect(
      await fetchGoogleBooksPages('', fetchImpl as unknown as typeof fetch),
    ).toBeNull();
    expect(fetchImpl).not.toHaveBeenCalled();
  });
});
...
describe('enrichPages', () => {
  ...
  it('swallows fetch errors and keeps pages null', async () => {
    ...
    try {
      const result = await enrichPages([
        makeCandidate({ externalId: '/works/OL3W' }),
      ]);
